...
2. Medir el riel de **3.3 V** de la placa → anotar: `V_3V3 = 3.285`
3. Medir el riel de **5 V** (VBUS del USB) → anotar: `V_5V = 5.079`
...
3. Anotar: nominal = 1000 Ω · medido = `994` Ω · error = `0.6` %

> ¿Qué tolerancia tiene? ¿El valor medido cae dentro de esa tolerancia?
Tiene una tolerancia de ±5%, debido a su ultima banda dorada. El valor de la medición está dentro de esa tolerancia.
...
- Elegir el valor comercial más cercano disponible: **68 Ω** o **100 Ω** (resistencia elegida: **100 Ω** )
...
- `V_R = 1.316` V
- `V_LED = 1.956` V 
- Verificación: `V_R + V_LED = 3.273`
- `I_real = 13.16` mA (¿razonable para un LED encendido? -> sí ya que está por debajo de los 20 mA ideales)

### Actividad 7 — (Opcional / lab abierto) Divisor de tensión

Armar un divisor con R1 = 10 kΩ y R2 = 4.7 kΩ. 
No teniamos resistencia de 4.7 kΩ, así que usamos 2 resistencias en serie. Una de 1 kΩ y la otra de 3.4 kΩ. Resultando con R2 = 4.4 kΩ, que es lo más cercano que tenemos a 4.7 kΩ.
$$V_{out} = V_{CC} \cdot \frac{R2}{R1 + R2}$$

- Predecir V~out~ teórico → 1.008 V
- Medir V~out~ real → 0.989 V
- Comparar: ¿qué lo aleja del ideal? (pista: tolerancia de resistencias, impedancia del multímetro)
La tolerancia de las resistencias. A pesar de esta tolerancia, el valor medido se acerca bastante al valor teórico, lo que indica que las resistencias están dentro de su rango de tolerancia.
...
- Frecuencia medida: `1` Hz
- Período medido: `1000` ms
- Delta: `δ = (f_medida - 1.0) / 1.0 × 100 = 0` %
...
| Medición | Valor |
|----------|-------|
| V riel 3.3 V | 3.285 V|
| V riel 5 V | 5.079 V|
| Resistencia nominal | 1000 Ω |
| Resistencia medida | 994 Ω|
| V_R (caída en R del LED) | 1.316 V|
| V_LED (caída en LED) | 1.956 V|
| I real (calculada) | 13.16 mA |
